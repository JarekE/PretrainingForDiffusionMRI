...
follows:
"""
# Now we have here: dataset with only 0, 1000 and 2000 bvalues. Here we generate a mask (ab hier nicht mehr nötig bei meinen daten)
...
"""
# csf (96,96,19), enhält array mit 1 csf  und 0 nicht, Rest identisch
...
"""
# soll ich diese Funktion nutzen? Nein, ich habe ja schon meine segmentierung
...
"""
# Hier wird es interessant! -> (2,4) array (Ich brauche nur meine Daten und die drei Masken)
...
"""
# produce unique bvals--> ich sollte diese Funktion nutzen (hier: (ubvals (0, 1000, 2000)
...
"""
# das hier sollte "2te Wahl" sein --> nicht nutzen
...
"""
# hier geht es mit "meiner" weiter --> genauso machen
mcsd_model = MultiShellDeconvModel(gtab, response_mcsd)

# Test of the peak_from_model function with this data

# PEAKS FROM MODEL (now 1/145)
# Good slice at 74
mcsd_peaks = peaks_from_model(model=mcsd_model,
                            data=data[:, :, :, :],
                            sphere=sphere,
...
np.save("peak_dirs.npy", np.float32(mcsd_peaks.peak_dirs))
# Normalized value of peak      peak_values (x,y,z, 5 value array)
# Number of peaks               to be calculated from peak_values
np.save("peak_values.npy", np.float32(mcsd_peaks.peak_values))